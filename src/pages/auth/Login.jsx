...
export default function Login() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4">
      ...
      <div className="card w-full max-w-[380px] p-8 shadow-2xl relative overflow-hidden">
        {/* Subtle decorative glow */}
        <div className="absolute -top-24 -right-24 w-48 h-48 bg-emerald-500/10 rounded-full blur-[60px] pointer-events-none"></div>

        <button className="text-slate-400 hover:text-emerald-400 mb-6 text-sm font-semibold flex items-center gap-1 transition-all" onClick={() => navigate("/role")}>
          ← Back
        </button>
        <h1 className="text-[32px] leading-10 font-bold mb-1 text-white">Sign in</h1>
        <p className="text-sm text-slate-400 mb-6 tracking-wide capitalize">{role} Login</p>
        ...
        <form onSubmit={handleLogin} className="space-y-5 relative z-10">
          ...
          <div
            className="text-sm font-semibold text-emerald-400 cursor-pointer hover:text-emerald-300 hover:underline mb-2 transition-colors"
            onClick={() => navigate("/forgot-password")}
          >
            Forgot password?
          </div>

          <button type="submit" disabled={loading} className="btn-primary w-full py-3.5 text-[16px] shadow-lg shadow-emerald-500/20">
